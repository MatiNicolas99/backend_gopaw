...
const getPetById = async (id) => {
  const consulta = "SELECT pet.id pet.pet_name, pet.birth_date, pet.type, owner.owner_name, veterinary.veterinary_name, pet.veterinary_id FROM pet JOIN owner ON pet.owner_id = owner.id JOIN veterinary ON pet.veterinary_id = veterinary.id WHERE owner.id = $1";
  const values = [id];
  const { rows } = await pool.query(consulta, values);
  console.log(rows);
  return rows;
